...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('esx_vehicleshopvip: ^1WARNING^7 plate character count reached, %s/8 characters.'):format(char))
	end
end)
...
MySQL.ready(function()
	...
	TriggerClientEvent('esx_vehicleshopvip:sendCategories', -1, Categories)
	TriggerClientEvent('esx_vehicleshopvip:sendVehicles', -1, Vehicles)
end)

RegisterServerEvent('esx_vehicleshopvip:setVehicleOwned')
AddEventHandler('esx_vehicleshopvip:setVehicleOwned', function (vehicleProps)
...
RegisterServerEvent('esx_vehicleshopvip:setVehicleOwnedPlayerId')
AddEventHandler('esx_vehicleshopvip:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
...
RegisterServerEvent('esx_vehicleshopvip:setVehicleOwnedSociety')
AddEventHandler('esx_vehicleshopvip:setVehicleOwnedSociety', function (society, vehicleProps)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_vehicleshopvip:sellVehicle')
AddEventHandler('esx_vehicleshopvip:sellVehicle', function (vehicle)
...
RegisterServerEvent('esx_vehicleshopvip:addToList')
AddEventHandler('esx_vehicleshopvip:addToList', function(target, model, plate)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(target)
	local dateNow = os.date('%Y-%m-%d %H:%M')

	if xPlayer.job.name ~= 'cardealer' then
		print(('esx_vehicleshopvip: %s attempted to add a sold vehicle to list!'):format(xPlayer.identifier))
		return
	end
...
ESX.RegisterServerCallback('esx_vehicleshopvip:getSoldVehicles', function (source, cb)
...
RegisterServerEvent('esx_vehicleshopvip:rentVehicle')
AddEventHandler('esx_vehicleshopvip:rentVehicle', function (vehicle, plate, playerName, basePrice, rentPrice, target)
...
RegisterServerEvent('esx_vehicleshopvip:getStockItem')
AddEventHandler('esx_vehicleshopvip:getStockItem', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_vehicleshopvip:putStockItems')
AddEventHandler('esx_vehicleshopvip:putStockItems', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_vehicleshopvip:getCategories', function (source, cb)
	cb(Categories)
end)

ESX.RegisterServerCallback('esx_vehicleshopvip:getVehicles', function (source, cb)
	cb(Vehicles)
end)

ESX.RegisterServerCallback('esx_vehicleshopvip:buyVehicle', function (source, cb, vehicleModel)
...
ESX.RegisterServerCallback('esx_vehicleshopvip:buyVehicleSociety', function (source, cb, society, vehicleModel)
	local vehicleData = nil
...
ESX.RegisterServerCallback('esx_vehicleshopvip:getCommercialVehicles', function (source, cb)
...
RegisterServerEvent('esx_vehicleshopvip:returnProvider')
AddEventHandler('esx_vehicleshopvip:returnProvider', function(vehicleModel)
	local _source = source
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			print(('esx_vehicleshopvip: %s attempted selling an invalid vehicle!'):format(GetPlayerIdentifiers(_source)[1]))
		end

	end)
end)

ESX.RegisterServerCallback('esx_vehicleshopvip:getRentedVehicles', function (source, cb)
...
ESX.RegisterServerCallback('esx_vehicleshopvip:giveBackVehicle', function (source, cb, plate)
...
ESX.RegisterServerCallback('esx_vehicleshopvip:resellVehicle', function (source, cb, plate, model)
	local resellPrice = 0
	...
	if resellPrice == 0 then
		print(('esx_vehicleshopvip: %s attempted to sell an unknown vehicle!'):format(GetPlayerIdentifiers(source)[1]))
		cb(false)
	end
				...
				if result[1] then -- does the owner match?
					...
					if vehicle.model == model then
						if vehicle.plate == plate then
							...
							print(('esx_vehicleshopvip: %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
							cb(false)
						end
					else
						print(('esx_vehicleshopvip: %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
						cb(false)
					end
...
ESX.RegisterServerCallback('esx_vehicleshopvip:getStockItems', function (source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)
...
RegisterServerEvent('esx_vehicleshopvip:setJobVehicleState')
AddEventHandler('esx_vehicleshopvip:setJobVehicleState', function(plate, state)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('esx_vehicleshopvip: %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)

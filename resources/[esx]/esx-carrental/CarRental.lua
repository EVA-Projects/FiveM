...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('carRental', 'Alquiler de coches')
	WarMenu.CreateSubMenu('closeMenu', 'carRental', '¿Estás seguro?')
	WarMenu.CreateSubMenu('carPicker', 'carRental', 'Elige un auto')
	WarMenu.CreateSubMenu('carInsurance', 'carRental', '¿Quieres comprar un seguro de auto?')
	WarMenu.CreateMenu('carReturn', 'Devolución del coche')
	WarMenu.SetSubTitle('carReturn', '¿Vas a devolver el auto?') 
	WarMenu.CreateMenu('arrestCheck', 'Alquiler de coches')
	WarMenu.SetSubTitle('arrestCheck', '¿Estás siendo arrestado actualmente?')
	
	while true do
		--Main menu
		if WarMenu.IsMenuOpened('carRental') then
			if WarMenu.MenuButton('¿Alquilar un coche?', 'carPicker') then
			elseif WarMenu.MenuButton('Seguro de auto', 'carInsurance') then
			...
			WarMenu.SetSubTitle('carRental', 'Alquile un auto por una tarifa')
		...
		elseif WarMenu.IsMenuOpened('carPicker') then
			if WarMenu.Button('Glendale | Por adelantado: 100€ | Diario: 100€') then
				SpawnVehicle("glendale")
				TriggerServerEvent("chargePlayer", 100)
				ESX.ShowNotification("Se le ha cobrado 100€ por su alquiler.")
				autoChargeAmount = 100
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Blista | Por adelantado: 200€ | Diario: 100€') then
				SpawnVehicle("blista2")
				TriggerServerEvent("chargePlayer", 200)
				ESX.ShowNotification("Se le ha cobrado 200€ por su alquiler.")
				autoChargeAmount = 100
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Primo | Por adelantado: 100€ | Diario: 100€') then
				SpawnVehicle("primo")
				TriggerServerEvent("chargePlayer", 100)
				ESX.ShowNotification("Se le ha cobrado 100€ por su alquiler.")
				autoChargeAmount = 100
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Intruso | Por adelantado: 250€ | Diario: 100€') then
				SpawnVehicle("intruder")
				TriggerServerEvent("chargePlayer", 250)
				ESX.ShowNotification("Se le ha cobrado 250€ por su alquiler.")
				autoChargeAmount = 100
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Banshee | Por adelantado: 1000€ | Diario: 300€') then
				SpawnVehicle("banshee")
				TriggerServerEvent("chargePlayer", 1000)
				ESX.ShowNotification("Se le ha cobrado 1000€ por su alquiler.")
		...
		elseif WarMenu.IsMenuOpened('carInsurance') then
			if WarMenu.Button('Sí | 200€') then
				TriggerServerEvent("chargePlayer", 200)
				damageInsurance = true
				ESX.ShowNotification("Gracias por comprar un seguro contra daños.")
				WarMenu.CloseMenu()
			elseif WarMenu.MenuButton('No', 'carRental') then
			end
		...
		if not isBlipCreated then 
			for _, v in pairs(pickupStation) do
				pickupBlip = AddBlipForCoord(v.x, v.y, v.z)
      			...
      			AddTextComponentString("Alquiler de coches")
	...
	while true do
		...
		if plate == " RENTAL " then
			if (IsVehicleDamaged(currentVehicle) and damageInsurance == false and damageCharge == false and canBeCharged == true) then
				...
				ESX.ShowNotification("Le han cobrado 500€ por dañar el automóvil. Comprar un seguro evitará que le cobren.")
			elseif (damageInsurance == true and IsVehicleDamaged(currentVehicle) and damageCharge == false) then
				ESX.ShowNotification("Ha dañado su vehículo, pero debido al seguro no se le cobrará.")
				damageCharge = true
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if isBeingCharged == true then
			...
			ESX.ShowNotification("Te han cobrado " .. autoChargeAmount .. "€ en otro día de su alquiler. Devuelva el vehículo para detener las tarifas.")
		end
	end
end)
...
function returnVehicle()
			...
			damageInsurance = false
			damageCharge = false
			ESX.ShowNotification("Gracias por devolver su alquiler. Por favor ven de nuevo!")
			local currentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
			SetEntityAsMissionEntity(currentVehicle, true, true)
			local x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(-1), false))

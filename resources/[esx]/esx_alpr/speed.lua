		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						radar.info = string.format("~y~Matricula: ~w~%s  ~y~Modelo: ~w~%s  ~y~Velocidad: ~w~%s mph", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						radar.info2 = string.format("~y~Matricula: ~w~%s  ~y~Modelo: ~w~%s  ~y~Velocidad: ~w~%s mph", bplate, bmodel, math.ceil(bvspeed))

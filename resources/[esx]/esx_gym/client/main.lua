...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Pulsa ~INPUT_CONTEXT~ para alquinar una ~b~bicicleta')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Ya tienes un ~r~vehiculo")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Pulsa ~INPUT_CONTEXT~ para abrir el menu de ~b~gimnasio~w~')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Pulsa ~INPUT_CONTEXT~ para hacer ~g~brazos')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							ESX.ShowNotification("Necesitas ~r~60 segundos ~w~para poder seguir haciendo ejercicio.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Necesitas ser miembro para hacer ~r~ejercicios")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Pulsa ~INPUT_CONTEXT~ para hacer ~g~pull-ups')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							Citizen.Wait(30000)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Necesitas esperar ~r~60 segundos ~w~para poder seguir haciendo ejercicio.")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Pulsa ~INPUT_CONTEXT~ para hacer ~g~pushups')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then				
							...
							Citizen.Wait(30000)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Tienes que esperar ~r~60 segundos ~w~para poder seguir haciendo ejercicio.")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Pulsa ~INPUT_CONTEXT~ para hacer ~g~yoga')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then	
							...
							Citizen.Wait(30000)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Necesitas descansar ~r~60 segundos ~w~para poder seguir haciendo ejercicio.")
						
							--TriggerServerEvent('esx_gym:trainYoga') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Necesitas ser miembro para hacer ~r~ejercicio")
						end
					elseif training == true then
						ESX.ShowNotification("Necesitas descansar...")
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Ya puedes hacer ejercicio...")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = 'Hacerse miembro', value = 'ship'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Estamos abiertos ~g~24~w~/7. Bienvenido!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Tienda',
            elements = {
				{label = 'Batido de proteinas (€6)', value = 'protein_shake'},
				{label = 'Agua (€1)', value = 'water'},
				{label = 'Sandwich de pavo (€2)', value = 'sportlunch'},
				{label = 'Powerade (€4)', value = 'powerade'},
				{label = 'Venda (€50)', value = 'bandage'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Hacerse miembro ($800)', value = 'membership'},
            }
        },
        function(data, menu)

    ...
    ESX.RegisterUsableItem(item_name, function(source)
        ...
        if lastTime and currentTime - lastTime < 10 then
            TriggerClientEvent("pNotify:SendNotification", source, {
                text = 'Espere al menos 10 segundos antes de reiniciar esta configuración.',
                type = "error",
                timeout = 2000,
                layout = "centerLeft"
            })
        ...
        if cops >= options[item_name].cops then
            ...
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'Para iniciar esta actividdad debe haber almenos '.. options[item_name].cops .. ' policias conectados.',
                type = "error",
                timeout = 5000,
                layout = "centerLeft"
            })
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'Número actual de policías de servicio: '.. cops ..'/'.. options[item_name].cops,
                type = "info",
                timeout = 6000,
                layout = "centerLeft"
            })
...
AddEventHandler("esx_receptury:statusSuccess", function(message, min, max, item)
    ...
    if(itemProps.limit < itemProps.count + amount) then
        ...
        TriggerClientEvent("pNotify:SendNotification", source, {
            text = 'Tienes el inventario lleno.',
            type = "error",
            timeout = 2000,
            layout = "centerLeft"
        })

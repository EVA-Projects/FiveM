...
function planePlate(source,plate)  
  local plate = plate
...
function planePrice(source,plate)  
  local plate = plate
  local user = getPlayerID(source)
...
AddEventHandler('ply_hangars:CheckForPlane', function(plate)
  ...
  local user = getPlayerID(source)
  local plane_plate = tostring(planePlate(source,plate))
...
AddEventHandler('ply_hangars:CheckForSelPlane', function(plate)
  ...
  local user = getPlayerID(source)
  local plane_plate = tostring(planePlate(source,plate))
  local plane_price = planePrice(source,plate)
  if plane_plate == plate then 

...
Citizen.CreateThread(function()
	...
	-------------------------------------------------------Poczatek TP
positions = {
    {{927.84, 44.43, 80.00, 0}, {906.69, -942.81, 44.42, 0}},} --
	...
	while true do
		...
		for i,location in ipairs(positions) do
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then
				alert("Pulsa E para entrar al casino")

				if IsControlJustReleased(1, 38) then
					if IsPedInAnyVehicle(player, true) then
						PlayerData = ESX.GetPlayerData()
						local elements = {
							{label = "Ticket Normal",  value = 'classicTicket'},
							{label = "Ticket Oro",  value = 'goldTicket'},
							{label = "Ticket Platino", value = "platiniumTicket"},
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CasinoCloak', {
							...
							elseif data.current.value == 'worker' then
								if PlayerData.job.name=='casino' then
									...
									menu.close()
									ESX.ShowNotification('~r~No eres un empleado del casino')
								end
							end
						...
						PlayerData = ESX.GetPlayerData()
						local elements = {
							{label = "Ticket Normal ["..Config.classicPrice.."€]",  value = 'classicTicket'},
							{label = "Ticket Oro ["..Config.goldenPrice.."€]",  value = 'goldTicket'},
							{label = "Ticket Platino ["..Config.platiniumPrice.."€]", value = "platiniumTicket"},
							{label = "Trabajador", value = "worker"}
						}
...
AddEventHandler('program-casino:hasEnteredMarker', function(zone)
	if zone =='CasinoActions' then
		...
		CurrentActionMsg  = "~r~[E] ~b~Abrir menú de administración del casino."
		CurrentActionData = {}
	elseif zone == "CasinoCloak" then
		CurrentAction     = 'CasinoCloak'
		CurrentActionMsg  = "~r~[E] ~p~Vestuario del casino"
		CurrentActionData = {}
	elseif zone == "CasinoFridge" then
		CurrentAction     = 'CasinoFridge'
		CurrentActionMsg  = "~r~[E] ~p~Nevera"
		CurrentActionData = {}
...
AddEventHandler('program-casino:openTicketMenuClient', function(worker)
	...
	if goldTicket then
		table.insert(elements, {label = "Mostrar el ticket de oro", value = 'showGolden'})
	end
	if platiniumTicket then
		table.insert(elements, {label = "Mostrar ticket platino", value = 'showPlatinium'})
	end
	if classicTicket then
		table.insert(elements, {label = "Mostrar ticket", value = 'showClassic'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pokazTicket', {
		...
		elseif data.current.value == 'showGolden' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene un ticket de oro.", worker)
		elseif data.current.value == 'showPlatinium' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene un ticket de platino.", worker)
		elseif data.current.value == 'showClassic' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene un ticket normal.", worker)
		end
...
function OpenCasinoActionsMenu()
	PlayerData = ESX.GetPlayerData()
	local elements = {
		{label = "Pide un ticket",  value = 'ticket'}
	}
	if closeStock == true then
		table.insert(elements, {label = "Armeria", value = 'weaponArmory'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'casino_mgmt', {
		...
		if data.current.value == 'ticket' then
			...
			if closestPlayer ~= -1 and closestDistance <= 3.0 then
				...
				ESX.ShowNotification('~r~No hay nadie alrededor')
			end

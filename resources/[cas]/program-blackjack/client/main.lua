		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil  then
				if CurrentAction == 'Croupier' then
					Croupier()
				elseif CurrentAction == 'Jugador 1' and isP1playing == false then
					Player1()
				elseif CurrentAction == 'Jugador 2' and isP2playing == false then
					Player2()
				end
			end
		end
...
AddEventHandler('program-casino:hasEnteredMarker', function(zone)
	if zone =='Croupier' then
		...
		CurrentActionMsg  = "~r~[E] ~b~Para abrir una mesa de BlackJack"
		CurrentActionData = {}
	elseif zone == "Player1" then
		CurrentAction     = 'Jugador 1'
		CurrentActionMsg  = "~r~[E] ~b~Unirse como el primer jugador"
		CurrentActionData = {}
	elseif zone == "Player2" then
		CurrentAction     = 'Jugador 2'
		CurrentActionMsg  = "~r~[E] ~b~Para unirse como segundo jugador"
		CurrentActionData = {}
	end
end)
...
AddEventHandler('program-casino:notifyPlayers', function(quantity)
	...
	if GetDistanceBetweenCoords(coords, Config.BlackjackSites.Croupier.Pos.x, Config.BlackjackSites.Croupier.Pos.y, Config.BlackjackSites.Croupier.Pos.z, true) < 29 then
		ESX.ShowAdvancedNotification("~p~BlackJack", "~p~Cantidad: ~g~"..quantity.."$", "~p~La mesa a comenzado, recuerda que los jugadores con ticket de Oro o Platino tienen prioridad ", "CHAR_BANK_BOL", 9)
	end
	Citizen.Wait(5000)
end)
...
AddEventHandler('program-blackjack:givecards', function()
	...
	local finish = false
	ESX.ShowNotification("~p~Te uniste a la mesa, tu primera carta es ~b~" ..cards[card][1].. " ~p~sobre el valor ~b~"..cards[card][2].."~n~~b~" ..wynik.." ~p~.")
	ESX.ShowNotification("~o~Para elegir una carta, haga clic en ~r~ [F]~o~ Para plantarse y mostrar cartas, haga clic en ~r~[X]")
	...
	while not finish do
		...
		if IsControlJustReleased(0, Keys["F"]) then
			...
			ESX.ShowNotification("~p~La carta elegida es ~b~" ..cards[card][1].. " ~p~sobre el valor ~b~"..cards[card][2].."~n~~p~tienes  ~b~" ..wynik.." ~p~.")
			if wynik>21 then
				if aces > 0 then
					aces = aces-1
					wynik = wynik - 10
					ESX.ShowNotification("~p~Tenías un as, así que no perdiste, solo contabas 1 punto en lugar de 11")
				else
					finish=true
					ESX.ShowNotification("~p~Tienes un total de más de 21 puntos, es decir ~b~" ..wynik.. "~p~, Perderás a menos que tu oponente también supere 21")
					aces = 0
				end
			end
...
function Croupier()
	...
	elseif PlayerData.job.name ~= 'casino' then
		ESX.ShowNotification("Debe ser un croupier para abrir la mesa.")
	elseif gameInProgress == true and PlayerData.job.name == 'casino' then
		ESX.ShowNotification("~b~El juego está actualmente en progreso para comenzar uno nuevo de forma remota pulsa ~r~[X]~b~ o espera hasta el final")
	end
...
function Player1()
	ESX.TriggerServerCallback('program-blackjack:checkStarting', function(result)
		if not result then
			ESX.ShowNotification('~r~¡La mesa no a comenzado, diríjase al croupier para pedirle que abra la mesa!')
		else
			if isPlatinium then
				local elements = {
					{label = "Jugar con dinero negro",  value = 'black'},
					{label = "Jugar con dinero",  value = 'money'}
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'wybierz_walute', {
					...
					elseif data.current.value == 'black' then
						ESX.TriggerServerCallback('program-blackjack:checkBlackMoney', function(result2)
							if not result2 then
								ESX.ShowNotification('~r~¡No tienes suficiente dinero para unirte a la mesa!')
							else
								isP1playing = true

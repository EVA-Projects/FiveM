                    ...
                    if(string.lower(plate) == localVehPlate)then
                        ...
                        if(vehicle ~= "locked")then
                            ...
                            if(time > timer)then
                                ...
                                TriggerEvent("ls:notify", "Tienes que esperar " .. (timer / 1000) .." segundos")
                            end
                        else
...
AddEventHandler("ls:getHasOwner", function(hasOwner, localVehId, localVehPlate, localVehLockStatus)
    if(not hasOwner)then
        ...
        TriggerEvent("ls:notify", "Este vehiculo no es tuyo.")
    end
end)
...
function Notify(text, duration)
	if(globalConf['CLIENT'].notification)then
		if(globalConf['CLIENT'].notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_LIFEINVADER", "CHAR_LIFEINVADER", true, 1, "Has usado las llaves", duration)
			DrawNotification_4(false, true)
		elseif(globalConf['CLIENT'].notification == 2)then
			TriggerEvent('chatMessage', 'Has usado las llaves' .. _VERSION, {255, 255, 255}, text)
		else
			return
		end
	else

	...
	if DoesEntityExist(pP) then
		...
		Citizen.CreateThread(function()
			...
			while jT > 0 and not eJE do
				...
				if jT % 30 == 0 then
					TriggerEvent('chatMessage', 'SYSTEM', { 0, 0, 0 }, jT .." más segundos hasta quedar en libertad.")
				end
				Citizen.Wait(500)
				...
				if D > 90 then
					...
					if D > 100 then
						...
						TriggerEvent('chatMessage', 'JUEZ', { 0, 0, 0 }, "Su tiempo en la cárcel se extendió por un intento de fuga ilegal.")
					end
				end
				jT = jT - 0.5
			end
			TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." a sido puesto en libertad.")

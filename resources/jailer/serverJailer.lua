...
local jailPassword = "adminguardianes99" --change this password to your liking and don't share it with the criminals ;-)
...
AddEventHandler('chatMessage', function(source, n, message)
    ...
    if(cm[1] == "/jailme") then
		...
		TriggerClientEvent('chatMessage', -1, 'JUEZ', { 0, 0, 0 }, GetPlayerName(source) ..' jailed for '.. jT ..' segundos')
		...
		if tablelength(cm) > 2 then
			if cm[2] == jailPassword then
				...
				if GetPlayerName(tPID) ~= nil then
					...
					TriggerClientEvent('chatMessage', -1, 'JUEZ', { 0, 0, 0 }, GetPlayerName(tPID) ..' jailed for '.. jT ..' secs')
				end
			else
				print("Incorrect jailPassword entered by ".. GetPlayerName(source))
			end

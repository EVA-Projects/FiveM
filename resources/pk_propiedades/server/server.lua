...
AddEventHandler('pk_casas:puerta', function(estado,nombrecasa)
	...
	if propietarioid ~= GetPlayerIdentifiers(source)[1] then 
		TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "Esta no es tu casa!.")
	elseif propietarioid == GetPlayerIdentifiers(source)[1] then
		MySQL.Sync.execute("UPDATE pk_casas SET estado = @estado WHERE propiedad = @propiedad", {['@estado'] = puerta,['@propiedad'] = nombrecasa})
			if estado == 0 then
				TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "You closed the door your house!")
				TriggerClientEvent('pk_casas:estadopuerta',-1, 0)
			elseif estado == 1 then
				TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "You opened the door of your house!")
				TriggerClientEvent('pk_casas:estadopuerta',-1, 1)
			end
	end
end)
...
------------------------------	
	...
	if canbuy == 1 then
		if sourceXPlayer.get('money') >= precio then
			...
			TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "Congratulations you bought "..nombrecasa.." for £"..precio)
		else
			TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "You don't have enought money!")
		end
	else
		TriggerClientEvent("chatMessage", source, "System", {255, 0, 0}, "You already have a property of this type!")
	end
...
local comprado = 1
	...
	if xPlayer.get('money') >= precio then
		...
		if tipo == armas then
			...
			TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "Congratulations, now you can store WEAPONS in your closet")
		elseif tipo == items then
			...
			TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "Congratulations, now you can store ITEMS in your closet")
		elseif tipo == dinero then
			...
			TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "Congratulations, now you can store Dirty MONEY in your closet")
		elseif tipo == garage then
			...
			TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "Congratulations, now you can store a vehicle in your garage")
		end
...
TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "You just put your property on sale for £"..precio)
end)
...
MySQL.Sync.execute("UPDATE pk_casas SET enventa = @enventa WHERE propiedad = @propiedad", {['@enventa'] = precio,['@propiedad'] = nombrecasa})
TriggerClientEvent("chatMessage", source, "PROPERTY", {255, 0, 0}, "You just got the sale of your property")
end)
------------------------------- GARAGE ------------------------------
    ...
    TriggerEvent('pk_casas:getSharedDataStore', propiedad, function(store)
      ...
      for i=1, #armario,1 do
        if armario[i].name == item then
          ...
          TriggerClientEvent('esx:showNotification', _source, "You don't have more inventory space")
		  else

...
function OpenBusinessMenu(business)
    ...
    ESX.TriggerServerCallback("esx_business:getStock", function(stock)
        ...
        table.insert(elements,{label = 'Comprar Stock - <span style="color:green;">€'..tostring(business["stock_price"])..'</span>', label_real = 'buystock', value = 1, type = 'slider', min = 1, max = 100})
        if business["owner"]==PlayerData.identifier then table.insert(elements,{label = 'Vender negocio - <span style="color:red;">€'..tostring(math.floor(business["price"]*Config.sell_percentage))..'</span>', value = 'sellbusiness'}) end
        if business["owner"]==PlayerData.identifier then table.insert(elements,{label = 'Lista empleados', value = 'employeelist'}) end
        table.insert(elements,{label = "Salir", value = 'exitmenu'})
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_boss',
        {
            ...
            if data.current.label_real == 'buystock' then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_confirm_stock', {
                    title    = ("Estas seguro que quieres comprar %s de stock por %s?"):format(data.current.value,business["stock_price"]*data.current.value),
                    align    = 'bottom-right',
                    elements = {
                        {label = 'No',  value = 'no'},
                        {label = 'Si', value = 'yes'},
                    ...
                    if data2.current.value == 'yes' then
                        ESX.TriggerServerCallback("esx_business:buyStock", function(success,time)
                            if success then
                                ESX.ShowNotification("Has comprado stock! Va a llegar en "..disp_time(time))
                            else ESX.ShowNotification("~r~No tienes suficiente dinero.") end
            ...
            elseif data.current.value == 'sellbusiness' then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_confirm_sellbusiness', {
                    title    = ("Estas seguro que quieres vender %s por %s?"):format(business["name"],math.floor(business["price"]*Config.sell_percentage)),
                    align    = 'bottom-right',
                    elements = {
                        {label = 'No',  value = 'no'},
                        {label = 'Si', value = 'yes'},
                    ...
                    if data2.current.value == 'yes' then
                        ESX.TriggerServerCallback("esx_business:sellBusiness", function(success)
                            if success then
                                ESX.ShowNotification("Has vendido tu negocio por ~g~€"..math.floor(business["price"]*Config.sell_percentage))
                            else ESX.ShowNotification("~r~Ocurrió un error") end
                        end,business["id"])
                    end
                    menu2.close()
                end, function(data2, menu2) menu2.close() end)
            ...
            elseif data.current.value=="employeelist" then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_choice', {
                    title    = business["name"]..' - Trabajadores',
                    align    = 'bottom-right',
                    elements = {
                        {label = 'Contratar trabajadores',  value = 'hire'},
                        {label = 'Despedir trabajadores', value = 'fire'},
                    }
                }, function(data2, menu2)
                    if data2.current.value=="hire" then
                        ESX.TriggerServerCallback("esx_business:getNewEmployeeList",function(newemployees)
                            ...
                            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_hire', {
                                title    = business["name"]..' - Contratar trabajadores',
                                align    = 'bottom-right',
                                elements = newemployeeselements
                            }, function(data3, menu3)
                                ESX.TriggerServerCallback("esx_business:hireEmployee",function(isok)
                                    ESX.ShowNotification(isok and "~g~Has contratado un nuevo trabajador!" or "~r~Ocurrió un error")
                    ...
                    elseif data2.current.value=="fire" then
                        ESX.TriggerServerCallback("esx_business:getEmployeeList",function(employees)
                            ...
                            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_fire', {
                                title    = business["name"]..' - Despedir trabajadores',
                                align    = 'bottom-right',
                                elements = employeeselements
                            }, function(data3, menu3)
                                ESX.TriggerServerCallback("esx_business:fireEmployee",function(isok)
                                    ESX.ShowNotification(isok and "~g~Has despedido un trabajador!" or "~r~Ocurrió un error")
...
function OpenBuyBusinessMenu(business)
    ...
    {
        title = "Comprar "..business["name"].." por €"..business["price"].."?",
        align = 'bottom-right',
        elements = {
            {label = 'Si', value = 'yes'},
            {label = 'No', value = 'no'}
        ...
        if data.current.value == 'yes' then
            ESX.TriggerServerCallback("esx_business:buyBusiness",function(status)
                if status==0 then
                    ESX.ShowNotification("Has comprado ~b~"..business["name"].."~s~!")
                elseif status==1 then
                    ESX.ShowNotification("~r~"..business["name"].."~s~ ya tiene un dueño!")
                else
                    ESX.ShowNotification("No tienes suficiente dinero para comprar ~b~"..business["name"].."~s~!")
                end
            end,business["id"])
        end
            ...
            if GetDistanceBetweenCoords(ppos,x,y,z,false)<Config.draw_distance then
                if business.owner==nil then
                    if GetDistanceBetweenCoords(ppos,x,y,z,false)<2.0 and not buy then
                        ESX.ShowHelpNotification('Pulsa ~INPUT_CONTEXT~ para comprar negocio')
                ...
                elseif business.owner==PlayerData.identifier or business.isemployee then
                    ...
                    if GetDistanceBetweenCoords(vector3(xx,yy,zz),GetEntityCoords(ped),false)<1.0 and not boss then
                        ESX.ShowHelpNotification('Pulsa ~INPUT_CONTEXT~ para abrir el menú de negocios')

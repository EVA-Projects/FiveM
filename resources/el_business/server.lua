...
function addMoney(identifier,money,business)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent('esx:showNotification', source, 'Has recivido ~g~'..tostring(money)..'€~s~ de ~b~'..business)
...
function noStock(identifier,business)
    ...
    if source ~= nil then
        TriggerClientEvent('esx:showNotification', source, '~r~'..business..'~s~ no tiene stock!')
    end
end
...
TriggerEvent('es:addGroupCommand', 'business', "superadmin", function(source, args, user)
    ...
    if #args>0 then
        if args[1]=="reload" then
            reloadServerData()
            reloadPlayersData()
            TriggerClientEvent('chat:addMessage', source, { args = { '^4Negocios', 'Datos cargados de la base de datos y sincronizados' } })
        elseif args[1]=="list" then
            TriggerClientEvent('chat:addMessage', source, { args = { '^4Negocios', 'La lista de negocios aparecerá en la consola F8' } })
        ...
        TriggerClientEvent('chat:addMessage', source, { args = { '^4Negocios', 'Parámetros incorrectos, los posibles parámetros son: ^2reload^7, ^2list^7, ^2create^7' } })
    end
...
ESX.RegisterServerCallback("esx_business:getNewEmployeeList",function(source,cb,business)
    ...
    if #players<1 then TriggerClientEvent('esx:showNotification', source, '~r~No hay jugadores en el servidor'); return end
    cb(players)
end)
...
ESX.RegisterServerCallback("esx_business:getEmployeeList",function(source,cb,business)
    ...
    if #idssql<1 then TriggerClientEvent('esx:showNotification', source, '~r~Tu lista de empleados está vacía'); return end
    ...
    if dataCache[business]["owner"]==identifier or isBusinessEmployee(dataCache[business]["employees"],identifier) then
        if xPlayer.getMoney()>=dataCache[business]["stock_price"]*amt then
            ...
            Citizen.CreateThread(function()
                ...
                TriggerClientEvent('esx:showNotification', source, 'Tu stock para ~b~'..dataCache[business]["name"]..'~s~ a llegado!')
            end)
            cb(true,time)
        else
            cb(false,0)

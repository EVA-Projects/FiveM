...
{626800628122779674, "^VIP | "}, -- 3
{577631197987995678, "^1T-Mod | "}, -- 4
{626074276612669440, "^1Mod | "}, -- 5
{626074195029393419, "^1Admin | "}, -- 6
{626072026280493056, "^6Programador | "}, -- 7
{626800405417820210, "^5Owner | "}, -- 8
}
	...
	if not string.find(args[1], "/") and not has_value(inStaffChat, source) then
		...
		if hasColors then
			...
			if not has_value(allowedColors, tonumber(roleNum)) then
				dontSend = true
				TriggerClientEvent('chatMessage', source, "^7[^1DiscordChatRoles^7] ^1No puedes usar colores en el chat si no eres VIP...")
			end
		end
		if hasRed then
			-- Check if they have required role
			if not has_value(allowedRed, tonumber(roleNum)) then
				dontSend = true
				TriggerClientEvent('chatMessage', source, "^7[^1DiscordChatRoles^7] ^1No puedes usar el color rojo en el chat si no eres STAFF...")
			end
		end
